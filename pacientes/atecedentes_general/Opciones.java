...
public class Opciones {
    ...
    static MongoCollection<Document> collection = database.getCollection("antecedentes_generales");

    public static boolean registrar(Sentencias uc) {
        try {
            Document document = new Document("id", uc.getId())
                    .append("general", uc.getGeneral());
    ...
    public static boolean actualizar(Sentencias uc) {
        try {
            ...
            Document update = new Document("$set", new Document("general", uc.getGeneral()));
